...
function init(bot, messageLimit) {
            ...
            db.query('SELECT * FROM buy_targets', function (err, rows) {
                ...
                if (rows.length > 0) {
                    for (i in rows) {
                        ...
                        if (params[0].ask <= target && messageLimit('buy', rows[i].chatid, +1)) {
                            bot.sendMessage(rows[i].chatid, 'Dude 💣 @' + rows[i].mention + ' \nIt\'s time to buy! Quick!! 💵💰💵💰💵💰💵 \nETH/USD:' + params[0].ask);
                        }
                    }
                }
            });
